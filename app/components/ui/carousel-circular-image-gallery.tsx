...
export function ImageGallery({ images, onSlideChange }: ImageGalleryProps) {
  ...
  return (
    <div className="flex items-center justify-center gap-3 sm:gap-5 lg:gap-8 py-6">
      <button
        className="flex-shrink-0 flex h-10 w-10 sm:h-12 sm:w-12 cursor-pointer items-center justify-center rounded-full border-2 border-white/20 bg-white/95 backdrop-blur-sm shadow-[0_8px_32px_rgba(0,0,0,0.12)] outline-none transition-all duration-300 ease-out hover:scale-110 hover:bg-white hover:border-white/40 hover:shadow-[0_12px_48px_rgba(0,0,0,0.18)] active:scale-95 focus-visible:ring-4 focus-visible:ring-white/50 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100"
        onClick={prev}
        disabled={disabled}
      ...
      <div className="relative h-[60vmin] w-[60vmin] max-h-[500px] max-w-[500px] overflow-hidden rounded-[20px] shadow-[0_2.8px_2.2px_rgba(0,0,0,0.02),0_6.7px_5.3px_rgba(0,0,0,0.028),0_12.5px_10px_rgba(0,0,0,0.035),0_22.3px_17.9px_rgba(0,0,0,0.042),0_41.8px_33.4px_rgba(0,0,0,0.05),0_100px_80px_rgba(0,0,0,0.07)]">
        {gsapReady &&
          images.map((image, i) => (
            <div
              key={image.url}
...
function GalleryImage({ uid, url, title, open, inPlace, id, onInPlace, total }: GalleryImageProps) {
  ...
  return (
    ...
    >
      <defs>
        <clipPath id={`${uid}_${id}_circleClip`}>
          <circle className="clip" cx="0" cy="0" r={circleRadius} ref={clip}></circle>
        </clipPath>
        <clipPath id={`${uid}_${id}_squareClip`}>
          <rect className="clip" width={width} height={height}></rect>
        </clipPath>
      </defs>
      <g clipPath={`url(#${uid}_${id}${inPlace ? "_squareClip" : "_circleClip"})`}>
...
function Tabs({ uid, images, onSelect }: TabsProps) {
  const gap = 10
  const circleRadius = 7
  const width = 400
  const height = 400
      ...
      {images.map((image, i) => (
        <g key={image.url} className="pointer-events-auto">
          ...
          <image
            ...
            href={image.url}
            clipPath={`url(#${uid}_tab_${i}_clip)`}
